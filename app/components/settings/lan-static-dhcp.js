...
export default class LanStaticDhcpComponent extends Component {
    ...
    get StaticEntries(){
        // return this.args.server.get('Pool').findBy('Alias', 'lan').StaticAddress.filter(item => !this.IDToBeDeleted.includes(item.id))
        const pool = this.args.server.get('Pool').findBy('Alias', 'lan');
        // const pool = this.store.peekAll('dhcpv4-server-pool').findBy('Alias', 'lan');
    ...
    get hasChanges() {
        let value = false;
        let arrOfNul = this.store.peekAll('dhcpv4-server-pool').findBy('Alias', 'lan').StaticAddress.filter(item => item.id == null);
    ...
    get hostsListing(){
        let model =  this.args.hosts.get('Host');
        ...
        return model.filter(item => {
            ...
            return current - item_date <= oneday
        })
    }
    ...
    @action
    Cancel() {
        this.args.server.get('Pool').findBy('Alias', 'lan').StaticAddress.forEach(e => {
    ...
    async addStaticAddr() {
        ...
        // newStaticAddr.save().finally(() => {
        //     this.staticDhcp = this.args.server.get('Pool').findBy('Alias', 'lan').StaticAddress;
        //     this.staticDhcp.pushObject(newStaticAddr);
        ...
        this.args.server.get('Pool').findBy('Alias', 'lan').StaticAddress.forEach((staticAddr) => {
            // console.log("Static DHCP ID = " + staticAddr.id);
            lastIndex = staticAddr.id.split('.')[3];
            ID = staticAddr.id.slice(0, -2);
        });

        if (ID == undefined) {
            ID = "Device.DHCPv4.Server.Pool.1.StaticAddress.";
            lastIndex = "0";
        }

        // console.log("ID = " + ID + " lastIndex = " + lastIndex);
        ...
        this.staticDhcp.pushObject(newStaticAddr);
        this.openEditModal(newStaticAddr);*/
    }

    async executeChanges() {
        const changePromises = 
                this.args.server.get('Pool').findBy('Alias', 'lan').StaticAddress
                .map(async e => {
